...
class MonteCarlo:
    ...
    def runSimAndReturnBest(self, states: [BoardInformation], iterations: int, actions, model, player, tau):
        ...
        actionId = np.argmax(np.array(all_actions_distribution.values()))
        action = list(all_actions_distribution.keys())[actionId]
        return action, new_distribution #, value, prior_distribution

    def runOneAndUpdate(self, root, player_color, discount, actions, model, isRoot=False):
        # print(root.state)
        ...
        if state_value == True:
            ...
            state_actions = set(actions).intersection(root.state.legal_moves)
                # print(state_actions)
            # else:
            #     state_actions = set(root.state.legal_moves)
            action_info = []
            priorsTotal = 0
            root.state.turn = player_color
            state_fen = root.state.fen()
            if state_fen not in self.visited:
                self.visited.add(state_fen)
                input = self.core.OHVETransform(root.state, player_color)
                inp = np.array([input])
                out, val = model.predict(inp)
                root.value = val[0]
                out = out[0]
                # print("INSIDE THIS")
                action_space = len(state_actions)
            ...
            root.transitions[action][1], val = self.runOneAndUpdate(root.transitions[action][1], not player_color, discount, root.transitions[action][1].state.legal_moves, model, False)
    ...
    def pickAction(self, action_values, total, c=1000):
        ucb_values = np.array([q+(c*(p*((total)**0.5)/(1+a_t))) for a_t, q, p in action_values])
        # best_vals = self.allmax(ucb_values)
        best_vals = np.argmax(ucb_values)
        return best_vals
        # if len(best_vals) > 1:
